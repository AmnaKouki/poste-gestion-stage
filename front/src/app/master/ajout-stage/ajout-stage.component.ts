...
export class AjoutStageComponent {
  ...
  generatePDF() {
    ...
    this.getStagiaire(this.savedStage.cinStagiaire).subscribe((stagiaire: any) => {

      if (!this.savedSuccessfully) {
        this.toastService.warning(' يرجى تثبيت التربص أولا');
      } else {
        const now = new Date();
        let dateHourNow = new Date().toLocaleString('fr');
  
        let dateNow = moment(now.toLocaleDateString()).format('DD/MM/YYYY');
        let dateDebutStage = moment(this.savedStage.dateDebut).format(
          'DD/MM/YYYY'
        );
        let dateFinStage = moment(this.savedStage.dateFin).format('DD/MM/YYYY');
        let institutFr = this.savedStage.institutFr;
        let cin = this.savedStage.cinStagiaire;
        let nomPrenom = stagiaire.prenomFr + ' ' + stagiaire.nomFr;
        let formatedDate = moment(stagiaire.dateNaissance).format(
          'DD/MM/YYYY'
        );
          ...
          pdfMake
            .createPdf({
              ...
              ...(this.getDocDefinition({
                ...
                lieuStage: this.savedStage.lieuStage.adresseFr,
                typeStage: this.savedStage.typeStage,
